...
from config.endpoints import google_endpoint
...
def process_and_format_response(response_data):
    ...
    for product in response_data:
        ...
        print(f"Price: {product['item_price']} (Total: {product['total_price']})")
...
def send_request_to_api(urls, API_KEY):
    ...
    try:
        response = requests.post(google_endpoint, headers=headers, json=payload)
